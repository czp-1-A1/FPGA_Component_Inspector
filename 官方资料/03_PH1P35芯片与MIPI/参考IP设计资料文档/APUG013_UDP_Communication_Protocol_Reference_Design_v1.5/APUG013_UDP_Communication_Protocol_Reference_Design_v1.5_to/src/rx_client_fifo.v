...
module rx_client_fifo#(
    ...
    parameter ADDR_WIDTH       = 11 //default 11,
)
  (
  ...
  wire        wr_addr_reload;
  reg  [ADDR_WIDTH:0] wr_start_addr;
  ...
  reg  [ADDR_WIDTH:0] wr_rd_addr;
  wire [ADDR_WIDTH+1:0] wr_addr_diff_in;
  reg  [ADDR_WIDTH:0] wr_addr_diff;
...
assign rd_store_frame_sync = wr_store_frame_tog_reg[1];
  ...
  // lower BRAM
  assign wr_en_l = wr_en & !wr_addr[ADDR_WIDTH];
  assign wr_en_u = wr_en & wr_addr[ADDR_WIDTH];
...
assign update_addr_tog_sync = update_addr_tog_reg[1];
  ...
  begin
     if (wr_sreset == 1'b1) begin
        ...
        if (update_addr_tog_sync_reg ^ update_addr_tog_sync) begin
           wr_rd_addr <= {rd_addr[ADDR_WIDTH:6], 6'd0};
        end
     end
  end
  ...
  begin
     if (wr_sreset == 1'b1) begin
        ...
        wr_addr_diff <= wr_addr_diff_in[ADDR_WIDTH:0];
     end
  end
  ...
  begin
     if (wr_sreset == 1'b1) begin
        ...
        if (wr_addr_diff[ADDR_WIDTH:4] == 0 && wr_addr_diff[3:2] != 2'b0) begin
           wr_fifo_full <= 1'b1;
        end
        else begin
           wr_fifo_full <= 1'b0;
  ...
  begin
     if (wr_sreset == 1'b1) begin
        ...
        if (wr_addr_diff == 'b0) begin
           wr_fifo_status <= 4'b0;
        end
        else begin
           wr_fifo_status[3] <= !wr_addr_diff[ADDR_WIDTH];
           wr_fifo_status[2] <= !wr_addr_diff[ADDR_WIDTH-1];
           wr_fifo_status[1] <= !wr_addr_diff[ADDR_WIDTH-2];
           wr_fifo_status[0] <= !wr_addr_diff[ADDR_WIDTH-3];
        end
     end
  end
